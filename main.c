...
void check_polarity(t_stack *stack_a, t_stack *stack_b, int a_index)
{
	if (a_index <= count_elems(stack_a) / 2)
	{
		if (stack_a[a_index].target < count_elems(stack_b) / 2)
			both_up(stack_a, stack_b, a_index);
